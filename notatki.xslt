...
<xsl:if test="position() != last()"><xsl:value-of select="local-name()"/>, ;<xsl:text>&#160;</xsl:text></xsl:if>
<xsl:if test="position()  = last()"><xsl:value-of select="local-name()"/>
</xsl:if>
</xsl:if></xsl:if>
<!--
<xsl:if test="position() != last()">
<xsl:if test="name() != 'born'"><xsl:value-of select="normalize-space(.)"/> ,   </xsl:if></xsl:if>
<xsl:if test="position()  = last()"><xsl:text>&#xd;</xsl:text><xsl:value-of select="normalize-space(.)"/>
</xsl:if> -->
</xsl:for-each>
<xsl:if test="local-name() != 'isbn'">
<xsl:if test="position() != last()"><xsl:value-of select="local-name()"/>, .<xsl:text>&#160;</xsl:text>
</xsl:if></xsl:if>
...
<xsl:for-each select="child::*[not(name()='child::*/child::isbn')]">
...
<xsl:if test="position()  = last()"><xsl:text>&#xd;</xsl:text><xsl:if test="local-name() != 'isbn'"><xsl:value-of select="normalize-space(.)"/>
</xsl:if> </xsl:if> 
</xsl:for-each>
</xsl:template> 
-->
